...
ap.add_argument("--jawgain",type=float,default=0.85); ap.add_argument("--orbit",action="store_true"); ap.add_argument("--armdown",type=float,default=1.25); ap.add_argument("--binddist",type=float,default=0.045); ap.add_argument("--closeup",action="store_true"); ap.add_argument("--cavity",action="store_true"); ap.add_argument("--armdamp",type=float,default=0.25)
a=ap.parse_args()
...
# bind every SHARP gaussian to nearest SMPL-X triangle (whole body)
c0,B0=tri_frames(V0,faces); _tree=cKDTree(c0)
dist,nn=_tree.query(Hm0)
kd=dist<a.binddist                                               # drop outliers (green edges, floaters) -> kills smear + fringe
Hm0,hq,Hs,ho,hc,nn=Hm0[kd],hq[kd],Hs[kd],ho[kd],hc[kd],nn[kd]
if a.cavity:   # dark gaussians behind the lips so an open jaw shows interior (needs z-tuning)
    ...
    cnn=_tree.query(back)[1]
    Hm0=np.concatenate([Hm0,back]); hq=np.concatenate([hq,cq]); Hs=np.concatenate([Hs,cs])
    ho=np.concatenate([ho,co]); hc=np.concatenate([hc,cc]); nn=np.concatenate([nn,cnn])
local=np.einsum('nij,nj->ni',np.transpose(B0[nn],(0,2,1)),Hm0-c0[nn]).astype(np.float32)
print("bound",len(Hm0),"(dropped",int((~kd).sum()),"outliers) to",len(faces),"tris")
...
for fi in range(F):
    ...
    Hm=c1[nn]+np.einsum('nij,nj->ni',B1[nn],local)
    R=np.einsum('nij,nkj->nik',B1[nn],B0[nn]); HqF=quat_mul(mat2quat(R),hq); HqF/=np.linalg.norm(HqF,axis=1,keepdims=True)
