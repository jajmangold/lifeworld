...
ap.add_argument("--jawgain",type=float,default=0.5); ap.add_argument("--orbit",action="store_true"); ap.add_argument("--armdown",type=float,default=1.25)
a=ap.parse_args()
...
keep=(green<0.03)&(ho>0.25)&(hm[:,2]<np.percentile(hm[:,2],55))   # person is in front; green bg is deeper
...
c0,B0=tri_frames(V0,faces); nn=cKDTree(c0).query(Hm0)[1]
local=np.einsum('nij,nj->ni',np.transpose(B0[nn],(0,2,1)),Hm0-c0[nn]).astype(np.float32)
print("bound to",len(faces),"triangles")
